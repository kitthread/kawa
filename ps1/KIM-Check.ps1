...
function sysconf {
    ...
    Copy-Item -Path $sys_path -Destination "c:\medistar\sysconf.txt"
    
    #sysconf auslesen
    $content = Get-Content "c:\medistar\sysconf.txt" | Where-Object {$_ -like "*MS4 = c:\MEDISTAR\para*"}

    #Prüfen ob der UNC-Pfad hinterlegt ist
    #$ms4 = Get-Content $content

    if ($content -like "*MS4 = c:\MEDISTAR\para*"){
        Show-Icon "success"
